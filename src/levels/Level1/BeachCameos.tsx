...
const SeagullSVG: React.FC<{ wingUp?: boolean }> = ({ wingUp }) => (
  <svg viewBox="0 0 48 32" className="w-14 h-9 filter drop-shadow" fill="none">
...
const SurferDuckSVG: React.FC<{ tilt?: boolean }> = ({ tilt }) => (
  <div className="flex flex-col items-center">
    <svg viewBox="0 0 64 64" className="w-24 h-24 filter drop-shadow" fill="none">
    ...
    <div
      className={`w-28 h-5 -mt-4 rounded-full bg-gradient-to-r from-fuchsia-500 via-yellow-400 to-cyan-400 border-2 border-white shadow-[0_0_16px_#06b6d4] flex items-center justify-between px-3 transition-transform duration-200 ${
    ...
    <svg viewBox="0 0 60 12" className="w-28 h-4 -mt-0.5" fill="none">
      <path d="M 2 10 Q 12 2, 22 10 Q 32 2, 42 10 Q 52 2, 58 10" stroke="#38bdf8" strokeWidth="3" strokeLinecap="round" />
      <circle cx="8" cy="4" r="2" fill="#e0f2fe" />
      <circle cx="50" cy="3" r="2" fill="#e0f2fe" />
    </svg>
  </div>
);
...
const PaparazziDuckSVG: React.FC<{ isFlashing?: boolean }> = ({ isFlashing }) => (
  <div className="relative flex flex-col items-center">
    {isFlashing && (
      <div className="absolute -top-14 -right-10 w-36 h-36 rounded-full bg-cyan-100/90 blur-2xl pointer-events-none animate-ping" />
    )}
    <svg viewBox="0 0 64 64" className="w-24 h-24 filter drop-shadow" fill="none">
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden select-none z-10">
      ...
      <div className="absolute bottom-2 left-2 sm:left-8 flex flex-col items-center">
        ...
        <span className="text-[9px] font-mono-rhythm text-yellow-300 bg-black/70 px-2 py-0.5 rounded-full font-bold -mt-1 shadow-md">
          PAPARAZZI
        </span>
      </div>
      ...
      <div className="absolute bottom-2 right-2 sm:right-8 flex flex-col items-center">
        ...
        <span className="text-[9px] font-mono-rhythm text-cyan-300 bg-black/70 px-2 py-0.5 rounded-full font-bold -mt-1 shadow-md">
          BEACH BAR
        </span>
      </div>
      ...
      {combo >= 10 && (
        <div className="absolute bottom-24 left-4 sm:left-16 flex flex-col items-center animate-fadeIn">
          <div className={`flex flex-col items-center ${isEven ? 'translate-y-1' : '-translate-y-2'}`}>
            <SecurityDuckSVG />
            <div className="w-16 h-4 -mt-2 bg-black rounded border border-yellow-400 flex items-center justify-center shadow-lg">
              <span className="text-[8px] font-bold text-yellow-300 tracking-wider">SECURITY</span>
            </div>
          </div>
        </div>
      )}
