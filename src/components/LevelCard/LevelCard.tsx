...
import { LevelScoreRecord, Collectibles, SaveData } from '../../services/SaveService';
...
export const LevelCard: React.FC<LevelCardProps> = ({
  ...
  const [isPlayable, setIsPlayable] = useState(() => UnlockService.isLevelPlayable(level.id, saveData));
  const [lockReason, setLockReason] = useState(() => UnlockService.getLockReason(level.id, saveData));
  const [countdown, setCountdown] = useState(() => UnlockService.getCountdown(level.id));

  useEffect(() => {
    const check = () => {
      const playable = UnlockService.isLevelPlayable(level.id, saveData);
      setIsPlayable(playable);
      setLockReason(UnlockService.getLockReason(level.id, saveData));
      if (!playable) {
  ...
  return (
    <div
      onClick={() => isPlayable && onSelect(level.id)}
    ...
    >
      ...
      {/* Center Status / Lock Reason / Countdown or Best Score */}
      <div className="my-6">
        {!isPlayable ? (
